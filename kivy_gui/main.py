...
class DataWindow(Screen):
    ...
    def update_datawindow_screen(self, wav_meta):
        """ Handles updates to the left hand side of the datawindow screen.

        This method parses the dict provided by scanner and provides text in a format
        that is appropriate for display using kivy .text methods. The method is split
        into three different sections to handle output generated by GSI/PSI commands,
        the MSI command, and the GLT command.

        Args:
            wav_meta (dict): metadata read from scanner, follows GSI_OUTPUT format from
                constants file.
        ...
        """
        # get the command used to generate data to pick appropriate processing
        cmd = wav_meta["cmd"]
